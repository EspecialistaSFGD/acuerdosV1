...
Public Class Form_registrosInversion
    Inherits System.Web.UI.Page

    Dim sw_asistente_DT As New DataTable
    Dim sw_asistente As New SW_SD_asistente_DA

    Private Sub Form_registrosInversion_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
        ...
        If Request.QueryString("gjXtIkEroS").ToString = "SD_SSFD" Then
            ...
            SDS_SD_P_selectListInversiones.ConnectionString = ConfigurationManager.ConnectionStrings(variableGlobalConexion.nombreCadenaCnx).ConnectionString
        Else
    ...
    Private Sub RadGrid1_DataBound(ByVal sender As Object, ByVal e As System.EventArgs) Handles RadGrid1.DataBound
        For Each item As GridDataItem In RadGrid1.Items
            ...
            Dim inversionEspacioId As Integer = currentRow.Row("inversionEspacioId").ToString
            ...
            Dim Link As ImageButton = item.FindControl("Link")

            Link.Attributes.Add("OnClick", "return verDetalle('" & inversionEspacioId.ToString.Trim & "');")
    ...
    Private Sub RadGrid1_DataBound(ByVal sender As Object, e As GridItemEventArgs) Handles RadGrid1.ItemDataBound
        For Each item As GridDataItem In RadGrid1.Items

            Dim inversionEspacioId As Integer = item("inversionEspacioId").Text
            Dim inversionID As Integer = item("inversionID").Text
            Dim tipoInversion As Integer = item("tipoInversion").Text

            Dim codigoInversion As String = item("codigoInversion").Text
            Dim TipoInv As String = item("TipoInv").Text
            Dim nomProyecto As String = item("nomProyecto").Text

            Dim comentario As ImageButton = item.FindControl("situacionGTC")

            comentario.Attributes.Add("onClick", "return frmSituacion('" + inversionEspacioId.ToString + "','" + inversionID.ToString + "','" + tipoInversion.ToString + "','" + codigoInversion.ToString + "','" + TipoInv.ToString + "','" + nomProyecto.ToString + "'); return true;")
            ...
            '    comentario.Attributes.Add("onClick", "return frmSituacion('" + inversionEspacioId.ToString + "','" + inversionID.ToString + "'); return true;")
            'End If
